...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if vagrant_config and vagrant_config['AWS-GAMIFICATION']['DEPLOY_TO_AWS']==true
    config.vm.box = "ubuntu_aws"
    config.vm.box_url = "https://github.com/mitchellh/vagrant-aws/raw/master/dummy.box"
    config.vm.provider :aws do |aws, override|
      aws.instance_type = "t1.micro"
      aws.access_key_id = vagrant_config['AWS-GAMIFICATION']['ACCESS_KEY_ID']
      aws.secret_access_key = vagrant_config['AWS-GAMIFICATION']['SECRET_ACCESS_KEY']
      aws.keypair_name = vagrant_config['KEYPAIR_NAME']
      aws.security_groups = vagrant_config['AWS-GAMIFICATION']['SECURITY_GROUPS']
      aws.ami = vagrant_config['AWS-GAMIFICATION']['AMI']
      aws.tags = {
        'Name' => 'Gamification',
        'Maturity' => 'Deployed'
      }
  ...
  if vagrant_config and vagrant_config['AWS-GAMIFICATION']['SKIP_SCRIPTS']

      if vagrant_config and vagrant_config['AWS-GAMIFICATION']['USE_LOCAL_REPO']==true
          config.vm.synced_folder "../django-gamification", "/vagrant/gamification-repo"
      end

  else
      config.vm.provision :shell, :path => "scripts/install_rvm.sh"
      config.vm.provision :shell, :path => "scripts/install_PIL.sh"
      config.vm.provision :shell, :path => "scripts/link_postgres_dir.sh"
      config.vm.provision :shell, :path => "scripts/install_ruby.sh"
      config.vm.provision :shell, :path => "scripts/install_lessc.sh"
      config.vm.provision :shell, :path => "scripts/link_badges_images.sh"
      ...
      config.vm.provision :chef_solo do |chef|
        ...
        chef.json = {
          :gamification => {
            :git_repo => {
              :location => "file:///vagrant/gamification-repo/"
            }
          }
        }
